...
def BNW_decode(outputs, entities, length):
    class Node:
        def __init__(self):
            self.THW = []                # [(tail, type)]
            self.NNW = defaultdict(set)   # {(head,tail): {next_index}}
    ...
    for instance, ent_set, l in zip(outputs, entities, length):
        ...
        for cur in reversed(range(l)):
            heads = [] # 这个数组记录了头的下标
            for pre in range(cur+1): # pre是可以等于cur的
                # THW
                if instance[cur, pre] > 1 or instance[cur, pre] < 0:
                    nodes[pre].THW.append((cur, instance[cur, pre])) # 为第i个节点加入尾巴和类型
                    heads.append(pre)
                # NNW
...
def EPW_decode(outputs, entities, length):
    class Node:
        def __init__(self):
            self.THW = []                # [(tail, type)]
            self.NNW = defaultdict(set)   # {(head,tail): {next_index}}
